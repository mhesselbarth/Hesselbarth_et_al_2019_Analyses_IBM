...
sa_default_pcf_b5 <- calc_pcf(data = sa_default, correction = "Ripley", r = r,
...
# sa_default_pcf_smaller <- calc_pcf(data = sa_default, correction = "Ripley", r = r,
#                                   window = window, smaller = 5) %>% 
#   dplyr::group_by(r) %>% 
#   dplyr::summarise(theo = mean(theo), 
#                    min = min(pcf), 
#                    max = max(pcf), 
...
sa_inc_5_pcf_b5 <- calc_pcf(data = sa_increased_5, correction = "Ripley", r = r,
                            window = window, bigger = 5) %>% 
...
sa_inc_10_pcf_b5 <- calc_pcf(data = sa_increased_10, correction = "Ripley", r = r,
                             window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   pcf = mean(pcf),
...
sa_dec_5_pcf_b5 <- calc_pcf(data = sa_decreased_5, correction = "Ripley", r = r,
                            window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   pcf = mean(pcf),
...
sa_dec_10_pcf_b5 <- calc_pcf(data = sa_decreased_10, correction = "Ripley", r = r,
                             window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   pcf = mean(pcf),
...
#### Plot results ####
sa_overall_pcf_b5 <- dplyr::bind_rows(sa_inc_5_pcf_b5, 
                                      sa_inc_10_pcf_b5, 
                                      sa_dec_5_pcf_b5,
                                      sa_dec_10_pcf_b5) %>% 
...
# create plot
sa_ggplot_pcf_b5 <- ggplot(data = sa_overall_pcf_b5) + 
  geom_ribbon(data = sa_default_pcf_b5,
...
suppoRt::save_ggplot(plot = sa_ggplot_pcf_b5, 
                     filename = "sa_ggplot_pcf_b5.png", path = "Figures/", 
                     dpi = 300, width = 30, height = 15, units = "cm", 
                     overwrite = overwrite)

#### Nearest neighbor distribution function ####
#### Calculate default result #### 
sa_default_nnd_b5 <- calc_nnd(data = sa_default, correction = "km", r = r, 
                             window = window, bigger = 5) %>% 
  dplyr::group_by(r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   min = min(km), 
...
#### Calculate changed parameters ####
sa_inc_5_nnd_b5 <- calc_nnd(data = sa_increased_5, correction = "km", r = r, 
                            window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   iso = mean(km),
...
sa_inc_10_nnd_b5 <- calc_nnd(data = sa_increased_10, correction = "km", r = r, 
                             window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   iso = mean(km),
...
sa_dec_5_nnd_b5 <- calc_nnd(data = sa_decreased_5, correction = "km", r = r, 
                            window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   iso = mean(km),
...
sa_dec_10_nnd_b5 <- calc_nnd(data = sa_decreased_10, correction = "km", r = r, 
                             window = window, bigger = 5) %>% 
  dplyr::group_by(parameter, r) %>% 
  dplyr::summarise(theo = mean(theo), 
                   iso = mean(km),
...
#### Plot results ####
sa_overall_nnd_b5 <- dplyr::bind_rows(sa_inc_5_nnd_b5, 
                                      sa_inc_10_nnd_b5, 
                                      sa_dec_5_nnd_b5,
                                      sa_dec_10_nnd_b5) %>% 
  dplyr::mutate(combined = forcats::as_factor(paste(direction, strength)),
...
sa_ggplot_nnd_b5 <- ggplot(data = sa_overall_nnd_b5) + 
  geom_ribbon(data = sa_default_nnd_b5, aes(x = r, ymin = min, ymax = max), 
              fill = "grey") +
...
suppoRt::save_ggplot(plot = sa_ggplot_nnd_b5, 
                     filename = "sa_ggplot_nnd_b5.png", path = "Figures/", 
                     dpi = 300, width = 30, height = 15, units = "cm", 
                     overwrite = overwrite)
