...
param_sampled <- purrr::map(seq_len(nrow(sobol_model$X)), 
                            function(x) as.numeric(sobol_model$X[x, ]))

# get the simulated model response #
simulation_results <- suppoRt::submit_to_cluster(calc_sobol_indiv, 
                                                 x = param_sampled, 
...
# flatten to vector #
simulation_results <- purrr::flatten_dbl(simulation_results)

# add the simulation results to the sobol instance #   
sensitivity::tell(sobol_model, simulation_results)
