...
plot_ppp_adult <- ggplot(data = as.data.frame(beech_1999_ppp)) +
  geom_raster(data = raster::as.data.frame(habitat_ras_fit, xy = TRUE),
              aes(x = x, y = y, fill = scaled)) +
  geom_point(aes(x = x, y = y, shape = "Individuals dbh > 10 cm"), alpha = 0.2) + 
  geom_polygon(data = plot_area_df, aes(x = x, y = y), fill = NA, col = "black") +
  scale_shape_manual(name = "", values = c("Individuals dbh > 10 cm" = 1)) +
...
suppoRt::save_ggplot(plot = plot_ppp_adult, 
                     file = "plot_ppp_abiotic_c.png", 
