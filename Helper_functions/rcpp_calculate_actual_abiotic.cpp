...
NumericVector rcpp_calculate_actual_abiotic(NumericMatrix matrix,
  ...
  for(int i = 0; i < nrow - 1; i++){
    ...
    const float abiotic_i = matrix(i, 4);
    ...
    for(int j = i + 1; j < nrow; j++){
      ...
      const float ci_temp_i = std::pow(dbh_j, alpha) * std::exp(-distance / 
                                       std::pow(dbh_j, beta));
      
      const float ci_temp_j = std::pow(dbh_i, alpha) * std::exp(-distance / 
                                       std::pow(dbh_i, beta));
    ...
    // result[i] = pot_i * mod * (1 - ci[i]) + pot_i * gamma * abiotic_i;
    result[i] = pot_i * mod * (((1 - ci[i]) + gamma * abiotic_i) / 2);
  ...
  //   matrix(nrow - 1, 3) * gamma * matrix(nrow - 1, 4);
  result[nrow - 1] = matrix(nrow - 1, 3) * mod * (((1 - ci[nrow - 1]) + gamma * matrix(nrow - 1, 4)) / 2);
